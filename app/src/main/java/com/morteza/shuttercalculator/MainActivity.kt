...
class MainActivity : AppCompatActivity() {
    ...
    private fun bindViews() {
        ...
        // NOTE: id in layout is inputInstallPrice (kept for compatibility)
        ...
        // textTotal now exists in layout
    ...
    private fun setupTextWatchers() {
        ...
        inputInstallPriceBase.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                val v = s.toString().replace(",", "").trim()
                val f = v.toFloatOrNull()
                if (f != null) {
                    previousInstallBase = f
                } else {
                    // restore last valid (with delay to avoid loop)
    ...
    private fun setupButtons() {
        buttonBasePrice.setOnClickListener {
            try {
                ...
                Toast.makeText(this, "صفحه قیمت‌های پایه یافت نشد", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        buttonReports.setOnClickListener {
           val i = Intent(this, ReportListActivity::class.java)
           startActivity(i)
        }

        // Save current report dialog and persist
